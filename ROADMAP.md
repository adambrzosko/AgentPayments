...
### `isBrowser()` misclassifies fetch()-based agents as browsers
Discovered building the live demo (`demo/`): Node's built-in `fetch()` (undici) always sends `sec-fetch-mode: cors` on every request — not settable, not overridable via the `headers` option. `isBrowser()` in all three SDKs checks `Sec-Fetch-Mode`/`Sec-Fetch-Dest` *first*, before the UA regex fallback, on the documented assumption that only real browsers send these headers (true when that check was written — Chrome 76+/Firefox 90+). That assumption no longer holds: any AI agent built on Node's `fetch()`, or any other spec-compliant HTTP client that sets Fetch Metadata headers, gets served the interactive JS challenge page instead of the machine-readable 402 JSON — the agent has no way to proceed. Confirmed directly: a Node `fetch()` call with a clearly non-browser User-Agent still received the HTML challenge page (status 200), while the identical request via `curl` or `http.request` correctly received 402 JSON. Likely a real contributor to the existing "ChatGPT and other LLM agents don't reliably read the 402 response instructions" issue in TODO.md — worth checking whether popular agent frameworks (LangChain.js, etc.) use `fetch()` under the hood. Needs a proper fix across Node/Edge/Python — options include weighting the UA check higher, or requiring a second signal (e.g. `Sec-Fetch-Dest: document`, which browsers set for top-level navigations, agents wouldn't) rather than either Sec-Fetch header alone.
